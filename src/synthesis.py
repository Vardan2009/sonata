...
def play_note(note: str, duration: float, ictx: InterpreterContext, actx: AudioContext):
    ...
    sustain_samples = max(note_abs_length - (attack_samples + decay_samples + release_samples), 0)
    ...
    samples: np.ndarray = wavefunc(2 * np.pi * freq * t).astype(actx.mixdown_type)
    samples *= envelope
    
    mixdown_result_length = max(len(actx.mixdown), actx.mixdown_ptr + note_abs_length)
    mixdown_result = np.zeros(mixdown_result_length, dtype=actx.mixdown_type)
    ...
    if max_val > 0:
        if np.issubdtype(mixdown_result.dtype, np.floating):
            ...
            mixdown_result = (mixdown_result.astype(np.float32) / max_val * max_int).astype(actx.mixdown_type)
